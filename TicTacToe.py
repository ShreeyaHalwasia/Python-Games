...
def new_game():
    ...
    for row in range(3):
        for column in range(3):
            buttons[row][column].config(text="", bg="dark blue", fg="black")
...
label = Label(text=player + " turn", font=('bell mt', 40), bg="black", fg="white")
label.pack(side="top")
...
for row in range(3):
    for column in range(3):
        buttons[row][column] = Button(frame, text="", width=5, height=2, font=('ink free', 25, 'bold'),  # Make the font bold
                                      command=lambda row=row, column=column: next_turn(row, column))
        buttons[row][column].grid(row=row, column=column)
        buttons[row][column].config(bg="black", fg="silver")  # Change text and background color
